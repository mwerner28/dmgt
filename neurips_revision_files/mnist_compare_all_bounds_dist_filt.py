...
def experiment(num_init_pts,
    ...
    if not file_exists(rare_acc_path):
        ...
        seive_taus=torch.zeros(len(trials),num_sel_rounds,2)
        ...
        rare_SEIVE_isoreg = train_isoreg(model, rare_val_loader) if is_isoreg else None
        common_SEIVE_isoreg = train_isoreg(model, common_val_loader) if is_isoreg else None
        ...
        for trial in trials: 
            ...
            for sel_round in range(num_sel_rounds):
                ...
                for agent in range(num_agents):
                    ...
                    agent_SEIVE_x, agent_SEIVE_y, _ = get_SEIVE_subsets(agent_stream_x,
                                                                        agent_stream_y,
                                                                        SEIVE_model,
                                                                        num_classes,
                                                                        is_isoreg,
                ...
                cent_SEIVE_x, cent_SEIVE_y, cent_min_max_taus = get_SEIVE_subsets(SEIVE_x,
                                                                                  SEIVE_y,
                                                                                  SEIVE_model,
                                                                                  num_classes,
                                                                                  is_isoreg,
                ...
                seive_taus[trial,sel_round] = cent_min_max_taus
                ...
                SEIVE_model = train(device,
                                    num_epochs,
                                    DataLoader(TensorDataset(cent_SEIVE_x, cent_SEIVE_y),
                                               batch_size=batch_size,
                                               num_workers=num_workers,
                                               shuffle=True),
                                    SEIVE_model)
                ...
                rare_SEIVE_isoreg = train_isoreg(SEIVE_model, rare_val_loader) if is_isoreg else None
                common_SEIVE_isoreg = train_isoreg(SEIVE_model, common_val_loader) if is_isoreg else None
...
def accuracy_plot(df, date_time, num_algs, num_sel_rounds):
    ...
    #
    #ax.xaxis.set_major_locator(ticker.FixedLocator(np.arange(num_sel_rounds)))
    #ax.xaxis.set_major_formatter(ticker.FixedFormatter([f'{i+1}' for i in range(num_sel_rounds)]))
...
if __name__ == "__main__":
    ...
    model_path = f'/home/eecs/mwerner/{args.dataset_name}_CODE/output_files/models/fed_init_model.pkl'
