...
# creates dataframe from main experiment data
def dist_dmgt_df(data, trials, num_sel_rnds):
    ...
    for trial in trials:
        df = df.append(pd.DataFrame({'trial':trial*torch.ones(num_sel_rnds+1),
                                     'sel_rnd':torch.arange(num_sel_rnds+1),
                                     'DIST_DMGT_all_acc':all_acc[trial,:,0].squeeze(),
                                     'RAND_all_acc':all_acc[trial,:,1].squeeze(),
                                     'DIST_DMGT_rare_acc':rare_acc[trial,:,0].squeeze(),
                                     'RAND_rare_acc':rare_acc[trial,:,1].squeeze(),
                                     'DIST_DMGT_rare_amnt':(torch.stack([x[:5].sum().int() for x in sizes[trial,:,0]])),
                                     'RAND_rare_amnt':(torch.stack([x[:5].sum().int() for x in sizes[trial,:,1]])),
                                     'DIST_DMGT_common_amnt':(torch.stack([x[5:].sum().int() for x in sizes[trial,:,0]])),
